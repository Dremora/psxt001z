...
wchar_t * char2wchar_t(s8 * oldstring) {
	u32 oldstringlength = strlen((const char *)oldstring);
	wchar_t * newstring = (wchar_t *)calloc(oldstringlength, 2);
	for (u32 i = 0; i < oldstringlength; i++) {
		newstring[i] = oldstring[i];
		*((char *)newstring + i*2 + 1) = 0;
	}
	return newstring;
}
...
u8 buffercmp(u8 *goodmd5, u8 *badmd5, u32 bsize) {
	for (u32 i = 0; i < bsize; i++) {
		if (*(goodmd5 + i) != *(badmd5 + i)) return 0;
	}
	return 1;
}
...
/*u8 *exe(FILE * file) {
	...
	fread(&lba, 4, 1, file);
	fseek(file, (2352 * lba) + 29, SEEK_SET);
	...
	if(buffer[0] != '\\') fseek(file, -1, SEEK_CUR);
	long i = -1;
	do fread(&buffer[++i], 1, 1, file); while(buffer[i] != ';');
...
u16 crc16(u8 *buf, int len) {
    u16 cksum = 0;
    for (int i = 0;  i < len;  i++) cksum = crc16_tab[((cksum>>8) ^ *buf++) & 0xFF] ^ (cksum << 8);
    return ~cksum;
}